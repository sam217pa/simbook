...
A key feature setting `msprime` apart from other coalescent simulations is a rich API for accessing the genealogies. We can even draw them, as shown in Figure \@ref(fig:drawtree). The code to make this figure is shown in \@ref(drawingtreescode).

```{r drawtree, echo=F, fig.cap="A tree with node times labelled. The SVG output is converted to png with [ImageMagick](www.imagemagick.org)."}
