...
The key difference between `ms` [@Hudson2002-oo] and `msprime` [@Kelleher2016-cb] is that the former stores the two trees in their entirety (using the pointer-based linked lists described in \@ref(linkedlists), while the latter compresses all of the information into node and edge tables:
